  ...
  return (
    <VenueLayout title="Dashboard">
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem' }}>
        <StatCard icon="🏛" value={venues.filter(v => v.isActive).length} label="Active listings" trend="+1 this month" />
        <StatCard icon="📅" value={bookings.length} label="Total bookings" trend="+12%" />
        <StatCard icon="💰" value={`EGP ${revenue.toLocaleString()}`} label="Est. revenue" sub="Based on confirmed bookings" trend="+8%" />
        <StatCard icon="⭐" value={pending} label="Pending requests" sub="Awaiting your response" />
      </div>
